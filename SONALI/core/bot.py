...
# Relative imports inside SONALI package
...
import ..config as config  # relative import for your config
...
async def enable_chat_cmd(client, message: Message):
    ...
    if member.status not in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
        return await message.reply_text("❌ Only admins can enable the chatbot.")
...
async def disable_chat_cmd(client, message: Message):
    member = await client.get_chat_member(message.chat.id, message.from_user.id)
    if member.status not in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
        return await message.reply_text("❌ Only admins can disable the chatbot.")
